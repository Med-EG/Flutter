...
class GetAllALertsTimesService {
  Future<List<AlertTimeModelForNotification>> GetAllLertsTimes(
      BuildContext context) async {
    PatientInfo? patient = BlocProvider.of<LoginCubit>(context).patient;
    try {
      List<dynamic> data = await Api().get(
          url:
              'https://api-medeg.online/api/medEG/alert-times/patient/${patient!.id}',
          token: patient.token);

      List<AlertTimeModelForNotification> times = [];
      for (int i = 0; i < data.length; i++) {
        times.add(AlertTimeModelForNotification.fromJson(data[i]));
      }
      return times;
    } catch (e) {
